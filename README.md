...
노코드 랜딩 페이지 구축 빌더와 나라장터 G2B API 연동 스크래핑 및 메일/구글 시트 알림 자동화 기능을 제공하는 **사내 통합 업무 플랫폼** 입니다.

---

## 목차
- [1. 전체 아키텍처 개요 및 인프라 흐름](#1-전체-아키텍처-개요-및-인프라-흐름)
- [2. 디렉토리 구조 및 주요 파일 기능 상세](#2-디렉토리-구조-및-주요-파일-기능-상세)
- [3. 핵심 동작 흐름 및 기능 상세 명세](#3-핵심-동작-흐름-및-기능-상세-명세)
- [4. 데이터베이스 엔티티 상세 스펙 및 컬럼 관계](#4-데이터베이스-엔티티-상세-스펙-및-컬럼-관계)
...
- **GCS**: 노코드 빌더로 완성된 정적 HTML 파일과 업로드된 리소스가 GCS 버킷에 배포되며, `Cloud CDN` + `Load Balancer`를 결합하여 서비스됩니다.

### (2) 스크래퍼 파이프라인 (자동/주기 실행)
1. **설정 동기화**: 관리자가 UI에서 스크래퍼 설정(알림 시각, 키워드, 메일 등)을 저장하면 백엔드 서버가 DB 테이블에 쓰고 **Google Cloud Scheduler**의 Cron Job을 실시간으로 업데이트합니다.
...
- **Problem (문제 상황)**
    ...
    - 비용 절감을 위해 GCE 내부에 MySQL을 직접 설치하여 운영하는 방식으로 전환했습니다.
    - 하지만 관리형 서비스가 제공하던 기능들을 직접 제어해야 하면서, **DB 서버의 시간 동기화, 사용자 권한 관리, 초기 테이블 셋업 등의 관리 복잡성이 증가**하는 이슈를 겪었습니다.
- **Solution (해결 방법)**
    - **테이블 관리:** `db-init.sql` 파일을 별도로 작성하여 초기 스키마 셋업과 기초 데이터 삽입 등 필요한 SQL 문을 문서화하고, DB 배포 시 이를 통해 테이블 생성이 자동화되도록 구성했습니다.
    - **DB 시간 관리:** 애플리케이션과 DB 간의 시간 불일치를 막기 위해, 호스트 VM의 OS 타임존을 한국 시간으로 동기화(`timedatectl set-timezone Asia/Seoul`)하고, MySQL 설정 파일(`my.cnf`)에 `default-time-zone='+09:00'`을 명시하여 데이터 정합성을 확보했습니다.
...
- **Solution (해결 방법)**
    ...
    - 사내 Google Workspace 도메인 관리자에게 요청하여, 메일 전송을 담당할 GCP 서비스 계정에 Gmail API 전송 권한을 부여받았습니다.
...
- **Problem (문제 상황)**
    - 애플리케이션은 Docker 컨테이너 환경으로 구축하고, MySQL은 호스트 VM 자체에 설치된 상태로 운영했습니다.
    - 컨테이너 이미지를 수정하고 새로 배포할 때마다 컨테이너 내부 IP가 계속 동적으로 변경되는 문제가 발생했습니다.
